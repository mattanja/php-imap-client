...
class Imap {
	...
	protected function formatMessage($id, $withbody = true) {
		...
		$email = array(
			'to' => isset($header->to) ? $this->arrayToAddress($header->to, true) : '',
			'from' => $this->toAddress($header->from[0], true),
			'date' => $header->date,
			'subject' => $subject,
			'uid' => $uid,
	 ...
	 */
	private function toAddress($headerinfos, $asArray = false) {
		$email = "";
		$name = "";
	 ...
	 */
	private function arrayToAddress($addresses, $asArray = false) {
		$addressesAsString = array();
		foreach ($addresses as $address) {
			$addressesAsString[] = $this->toAddress($address, $asArray);
		}
	...
	private function getBody($uid) {
		...
		// Always also get the text part
		$text = $this->get_part($this->imap, $uid, "TEXT/PLAIN");
